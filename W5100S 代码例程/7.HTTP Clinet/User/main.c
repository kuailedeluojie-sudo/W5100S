...
int main(void)
{
  ...
  printf("\r\n W5100S 端口号：%d。\r\n",local_port);      
  printf(" OneNET服务器IP为:%d.%d.%d.%d。\r\n",onenet_server_ip[0],onenet_server_ip[1],onenet_server_ip[2],onenet_server_ip[3]);
  printf(" OneNET服务器端口:%d。\r\n",80);
  printf(" 请登录对应OneNET账号观察是否接收到数据。\r\n");
  while(1)                               
  {
    do_http_client();                   // HTTP 连接OneNET平台上传数据
  }
}
